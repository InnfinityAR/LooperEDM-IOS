...
-(void)setUpSomeDatas{
    ...
    CGFloat degree = (270/(menuCount+1.6))*(M_PI/180);
    degree = 0.7853982;
    //参考点的坐标
    CGPoint originPoint = self.mainView.center;
    for (NSInteger i = 0; i < menuCount; i++) {
        CGFloat cosDegree = cosf(degree * (i+2));
        CGFloat sinDegree = sinf(degree * (i+2));
       ...
       // item.backgroundColor = menuColor;
        ...
        UIImageView *imageV = [[UIImageView alloc] initWithFrame:CGRectMake(0,0,70*DEF_Adaptation_Font*0.5,99*DEF_Adaptation_Font*0.5)];
        imageV.layer.cornerRadius = item.bounds.size.width / 2;
        
        if(i==0){
            [imageV setImage:[UIImage imageNamed:@"btn_ticket.png"]];
        }else if(i==1){
            [imageV setImage:[UIImage imageNamed:@"btn_calendar.png"]];
        }else if(i==2){
            [imageV setImage:[UIImage imageNamed:@"btn_activity_looper.png"]];
        }
...
-(void)tapToSwitchOpenOrClose{
    if ( [self.menuDelegate respondsToSelector:@selector(menuDidSelected:)]) {
         [self.menuDelegate menuDidSelected:100];
    }
    ...
    if (isOpened == NO) {
        ...
        for (UIView *item in Menus) {
            ...
            [UIView animateWithDuration:1.0f delay:0.05*item.tag usingSpringWithDamping:0.4f initialSpringVelocity:0.0 options:UIViewAnimationOptionCurveEaseIn | UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionAllowUserInteraction animations:^{
                ...
                cross.transform = CGAffineTransformMakeRotation(90*(M_PI/180));
