...
- (void)viewDidLoad {
    ...
    //创建视频录制对象
    self.recorder = [[PKShortVideoRecorder alloc] initWithOutputFilePath:self.outputFilePath outputSize:CGSizeMake(kScreenWidth, kScreenHeight)];
    //通过代理回调
    ...
    self.progressBar = [[PKShortVideoProgressBar alloc] initWithFrame:CGRectMake(0, kScreenHeight-10, kScreenWidth, 5) themeColor:self.themeColor duration:self.videoMaximumDuration];
...
- (void)sendButtonAction  {
    ...
    [self.refreshButton addTarget:self action:@selector(sendVideo) forControlEvents:UIControlEventTouchUpInside];
    [self.playButton addTarget:self action:@selector(refreshView) forControlEvents:UIControlEventTouchUpInside];
}
...
//            }];
            ...
            UIImage *playImage = [UIImage imageNamed:@"PK_Delete"];
            ...
            UIImage *refreshImage = [UIImage imageNamed:@"PK_Play"];
