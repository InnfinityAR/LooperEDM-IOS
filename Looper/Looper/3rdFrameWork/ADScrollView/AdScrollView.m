...
{
    if (_call_back) {
        _call_back(_pageControl.currentPage);
    }
}
...
- (void)setImageNameArray:(NSArray *)imageNameArray
{
    ...
    leftImageIndex = imageNameArray.count-1;
    centerImageIndex = 0;
    rightImageIndex = 1;
    ...
    //[_leftImageView sd_setImageWithURL:_imageNameArray[leftImageIndex] placeholderImage:nil];
    //[_centerImageView sd_setImageWithURL:_imageNameArray[centerImageIndex] placeholderImage:nil];
    //[_rightImageView sd_setImageWithURL:_imageNameArray[rightImageIndex] placeholderImage:nil];
...
- (void)setPageControlShowStyle:(UIPageControlShowStyle)PageControlShowStyle
{
    ...
    else if (PageControlShowStyle == UIPageControlShowStyleCenter)
    {
        ...
        NSLog(@"HIGHT----%f,----%f",HIGHT,HIGHT+UISCREENHEIGHT);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if (self.contentOffset.x == 0)
    {
        ...
        if (leftImageIndex == -1)   //当没有前一张时，将最后一张移过来
        {
    ...
    else if(self.contentOffset.x == UISCREENWIDTH * 2)
    {
        ...
        if (centerImageIndex == _imageNameArray.count)
        {
            centerImageIndex = 0;
        }
        if (rightImageIndex == _imageNameArray.count)
        {
            rightImageIndex = 0;
        }
    }
    ...
    [_leftImageView setImage: [UIImage imageNamed:_imageNameArray[leftImageIndex]]];
    _leftAdLabel.text = _adTitleArray[leftImageIndex];
    
    [_centerImageView setImage: [UIImage imageNamed:_imageNameArray[centerImageIndex]]];
    _centerAdLabel.text = _adTitleArray[centerImageIndex];
    
    
    [_rightImageView setImage: [UIImage imageNamed:_imageNameArray[rightImageIndex]]];
