...
@property (nonatomic, strong) UIImageView *tempV;
...
#define kCalendarBasicColor [UIColor colorWithRed:30.0 / 255.0 green:30.0 / 255.0 blue:35.0 / 255.0 alpha:0.9]

#define kCalendarColor [UIColor colorWithRed:219.0 / 255.0 green:154.0 / 255.0 blue:2.0 / 255.0 alpha:0.9]
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if ([super initWithFrame:frame]) {
        ...
        self.bounces = NO;
        self.delegate = self;
        self.subArray = [[NSMutableArray alloc] initWithCapacity:50];
...
- (void)setupCollectionViews {
    ...
    _collectionViewL.backgroundColor = kCalendarBasicColor;
    ...
    _collectionViewM.backgroundColor = kCalendarBasicColor;
    ...
    _collectionViewR.backgroundColor = kCalendarBasicColor;
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == _collectionViewL) {
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            cell.todayLabel.textColor = [UIColor colorWithRed:217/255.0 green:217/255.0 blue:217/255.0 alpha:1.0];
        
            
            if((firstWeekday+indexPath.row - firstWeekday + 1)%7==0  ||(firstWeekday+indexPath.row - firstWeekday + 1)%7==1 ){
                cell.todayLabel.textColor = [UIColor greenColor];
                
            }
            
            if([cell.todayLabel.text intValue]==7){
                
                cell.selImg.hidden=false;
            ...
            if ((monthInfo.month == [[NSDate date] dateMonth]) && (monthInfo.year == [[NSDate date] dateYear])) {
                if (indexPath.row == [[NSDate date] dateDay] + firstWeekday - 1) {
                    cell.todayCircle.backgroundColor = kCalendarColor;
        ...
        // 补上前后月的日期，淡色显示
            ...
            cell.todayLabel.textColor =  [UIColor colorWithRed:128/255.0 green:128/255.0 blue:128/255.0 alpha:1.0];
            cell.todayCircle.backgroundColor = [UIColor clearColor];
...
-(void)removeALL{
    ...
    [self.tempV removeFromSuperview];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.didSelectDayHandler != nil) {
        ...
        UIImageView *selImg = [[UIImageView alloc] initWithFrame:CGRectMake( 8*DEF_Adaptation_Font,4*DEF_Adaptation_Font, 57*DEF_Adaptation_Font*0.5, 57*DEF_Adaptation_Font*0.5)];
        selImg.image = [UIImage imageNamed:@"icon_calendar_click.png"];
        selImg.tag==100;
        [cell addSubview:selImg];
        self.tempV = selImg;
